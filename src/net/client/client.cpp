...
class Client::Impl {
   public:
    ...
    ~Impl() = default;  // FdGuard handles socket cleanup
    ...
    [[nodiscard]] bool connected() const noexcept {
        return static_cast<bool>(socket_fd_);
    }
   ...
   private:
    Response execute(const Request& req) {
        ...
        if (!protocol_->write_request(socket_fd_.get(), req)) {
            disconnect();
            throw std::runtime_error("failed to send request");
        }

        auto resp = protocol_->read_response(socket_fd_.get());
        if (!resp) {
            disconnect();
