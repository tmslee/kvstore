...
public:
    ...
    /*
        ...
        if someone truly needs a copy, they should be explicit abt it (we can provide clone() method)
    */
    KVStore(const KVStore&) = delete;
    KVStore& operator=(const KVStore&) = delete;
    /*
        standard containers (i.e. std::vector) check if your move ops are noexcept before reallocation
        if your ctor can throw, vector::push_back will copy instead of move for exception safety
        noexcept moves enable optimal container behavior
    */
    ...
    /*
        ...
        get: same lookup concerns as above + we return std::optional<std:;string> which means constructing a new std::string -> allocation = can throw.
    */
    ...
    /*
        ...
        other use cases: parsing/tokenizing, passing string literals, function returns when lifetime is guaranteed - reutrning a view into static data or member data in single-threaded context
