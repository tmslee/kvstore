...
struct Config{
    ...
    static std::optional<Config> parse_pargs(int argc, char* argv[]);

    //merge: CLI overrides file
    static Config merge(const Config& file_config, const Config& cli_config, const Config& defaults);
};
